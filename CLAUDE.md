...
- **当前状态**：已完成登录页、主布局（layout）、员工管理、分类管理、菜品管理；剩余 4 个页面待开发（见第 6 章清单）。
...
**📌 当前进度**：进行中页面 = 套餐管理（开发中）；菜品管理已完成（用户审查通过并已提交 git）。每次组件完成提交后，同步更新本行与第 6 章清单。
...
- [ ] 3. 套餐管理（`views/setmeal`）——进行中——依赖：分类、菜品
