...
- **当前状态**：已完成登录页、主布局（layout）、员工管理（完整增删改查分页模板）；剩余 6 个页面待开发（见第 6 章清单）。
...
**📌 当前进度**：进行中页面 = 分类管理（代码已完成，用户审查中）；两个 P0 小任务已完成（待提交）。每次组件完成提交后，同步更新本行与第 6 章清单。
...
- [ ] 1. 分类管理（`views/category`）——待开发——依赖：无
- [ ] 2. 菜品管理（`views/dish`）——待开发——依赖：分类
- [ ] 3. 套餐管理（`views/setmeal`）——待开发——依赖：分类、菜品
- [ ] 4. 订单管理（`views/order`）——待开发——依赖：菜品、套餐
...
| # | 问题 | 位置 | 优先级 / 建议时机 |
|---|---|---|---|
| 1 | layout 的 `handleCommand` 未定义，退出登录/修改密码无效 | `src/views/layout/index.vue` | ✅ 已修复（待提交） |
| 2 | 金黄主题变量覆盖 | `src/assets/main.css` | ✅ 已撤销：试装后用户觉得金黄按钮不好看，改回默认蓝（见 7.2），待提交 |
